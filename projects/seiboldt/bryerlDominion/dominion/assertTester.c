...
int assertTrue(int PF, char* test, int PNtest, int player, int card)
{
    ...
    if(card == 9) {strncpy(cardA, "steward", (sizeof cardA)-1);}
    ...
    if(PNtest == 5){
        strncpy(PNtest_L, "does increase and", (sizeof PNtest_L)-1);
    }
    else{
    ...
    {   char strFail[80];
        ...
        strncpy(F1, "FAILURE: Testing found bug in ", (sizeof F1)-1);
        strcpy(strFail, F1);
...
int assertTrue1(int PF, char* test, int PNtest, int player, int card)
{
    if(PF)
    {return 1;}
    else
