...
int callCutpurse(struct gameState *state, int i, int j, int k, int currentPlayer, int handPos) {
    ...
    for (i = 0; i < state->numPlayers; i++)
    {
        if (i != currentPlayer)
        {
            for (j = 0; j < state->handCount[i]-1; j++)
            {
...
int
callAdventurer(struct gameState *state, int currentPlayer, int *temphand, int drawntreasure, int cardDrawn,
               int z) {
...
int callMinion(struct gameState *state, int handPos, int currentPlayer, int choice1, int choice2, int i, int j){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    if(card == smithy){
        if(callSmithy(i, currentPlayer, state, handPos)  == 0)
        {return 0;}
        else return -1;
    }
    if(card == adventurer){
        if(callAdventurer(state, currentPlayer, temphand, drawntreasure, cardDrawn, z)  == 0)
        {return 0;}
        else return -1;
    }
    if(card == great_hall){
        if(callGreatHall(currentPlayer, state, handPos)  == 0)
        {return 0;}
        else return -1;
    }
    if(card == minion){
        if(callMinion(state, handPos, currentPlayer, choice1, choice2, i, j)  == 0)
        {return 0;}
        else return -1;
    }
